...
OUTPUT_COMPACT = RES_DIR / "words_compact.json"
OUTPUT_WORDS = RES_DIR / "words.json"
OUTPUT_WEB = ROOT_DIR / "web" / "public" / "data" / "words.json"

sys.path.insert(0, str(ROOT_DIR))
from scripts.split_and_manage_words import THEMES_MAP
...
def prepare_compact_data():
    ...
    themes = {}
    for idx, th_title in THEMES_MAP.items():
        themes[str(idx)] = th_title
    ...
    OUTPUT_WEB.parent.mkdir(parents=True, exist_ok=True)
    
    for target in [OUTPUT_COMPACT, OUTPUT_WORDS, OUTPUT_WEB]:
        print(f"✍️ Writing {len(compact_words)} compact words to {target}...")
        with open(target, "w", encoding="utf-8") as f:
            json.dump(compact_payload, f, ensure_ascii=False, separators=(',', ':'))
        
    orig_size = WORDS_FILE.stat().st_size
    new_size = OUTPUT_COMPACT.stat().st_size
